const { AuthenticationError, UserInputError, ForbiddenError, withFilter } = require('apollo-server');
    ...
    Mutation: {
        ...
        reactToMessage: async (_, { uuid, content }, { user, pubsub }) => {
            ...
            try {
                ...
                const message = Message.findOne({ where: { uuid }})
    ...
    Subscription: {
        newMessage: {
            subscribe: withFilter(
                ...
                ({ newMessage }, _, { user }) => {
                    if(newMessage.from === user.username || newMessage.to === user.username) return true
                    return false
                }
            )
        },
